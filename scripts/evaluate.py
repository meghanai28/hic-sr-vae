...
def per_diagonal_correlation(pred, target, max_diag=100):
    N = min(pred.shape[0], target.shape[0])
    corrs = []
    for d in range(min(max_diag, N)):
        p, t = np.diag(pred, k=d), np.diag(target, k=d)
        if len(p) < 3:
            corrs.append(np.nan); continue
        sp, st = np.std(p), np.std(t)
        if sp < 1e-10 or st < 1e-10:
            corrs.append(0.0)
        else:
...
def main():
    ...
    for lr_b, hr_b in tqdm(loader, desc="eval"):
        ...
        for b in range(lr_b.size(0)):
            if 0 < args.limit <= count: break
 
            lr_np = lr_b[b, 0].cpu().numpy()
            sr_np = pred[b, 0].cpu().numpy()
            hr_np = hr_b[b, 0].cpu().numpy()
 
            sr_c, hr_c = center_crop_to_match(torch.from_numpy(sr_np), torch.from_numpy(hr_np))
            sr_c, hr_c = sr_c.numpy(), hr_c.numpy()
            lr_c, _ = center_crop_to_match(torch.from_numpy(lr_np), torch.from_numpy(sr_c))
            lr_c = lr_c.numpy()
 
            m = mse_np(sr_c, hr_c)
            s = ssim_np(sr_c, hr_c)
            dc = per_diagonal_correlation(sr_c, hr_c)
            mc = float(np.nanmean(dc[:50]))
            metrics.append(dict(mse=m, ssim=s, diag_corr=mc))
            ...
            fig.suptitle(f"MSE={m:.4f}  SSIM={s:.3f}  DiagCorr={mc:.3f}", fontsize=10)
    ...
    if metrics:
        ...
        print(f"  DiagCorr: {np.mean([m['diag_corr'] for m in metrics]):.3f}")
